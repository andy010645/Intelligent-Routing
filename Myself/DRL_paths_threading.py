...
import sys
sys.path.insert(0,'./RoutingGeant/DRL/dRSIR/23nodos')
# sys.path.insert(0,'./RoutingGeant/DRL/dRSIR/32nodos')
# sys.path.insert(0,'./RoutingGeant/DRL/dRSIR/64nodos')
# sys.path.insert(0,'./RoutingGeant/DRL/dRSIR/48nodos')
import time
#mport bot
import json,ast
import csv
...
def get_paths_base():
    file_base = './RoutingGeant/DRL/dRSIR/'+str(num_nodes)+'nodos/paths_weight.json'
...
def calc_all_stretch(cont):
    ...
    a = time.time()
    with open('./stretch/'+str(cont)+'_stretch.csv','w') as csvfile:
...
def get_all_paths(episode_rewards, episode_states_all, episode_duration_all, episodes):
    ...
    file = './RoutingGeant/DRL/dRSIR/'+str(len(env.topo_nodes))+'nodos/k_paths.json'
...
def DRL_thread(): #cambiar para que lo llame a drl
    ...
    file_info_eps = "./episode_info.txt"
